...
async function handlePost(req, res, kvConfig) {
  ...
  const modes =
    payload?.modes && typeof payload.modes === "object" ? payload.modes : {};

  await writeManyImageGroupsToKv(kvConfig, { modes });

  res.status(200).json({
    counts: {
      shows: Object.values(modes).reduce(
        (count, groups) => count + Object.keys(groups?.shows || {}).length,
        0,
      ),
      movies: Object.values(modes).reduce(
        (count, groups) => count + Object.keys(groups?.movies || {}).length,
        0,
      ),
      seasons: Object.values(modes).reduce(
        (count, groups) => count + Object.keys(groups?.seasons || {}).length,
        0,
      ),
    },
  });

...
async function handleGet(req, res, kvConfig) {
  ...
  const { lists } = await readManyListTranslationEntriesFromKv(
    kvConfig,
    listIds,
  );
...
async function handlePost(req, res, kvConfig) {
  ...
  const incomingLists =
    payload?.lists && typeof payload.lists === "object" ? payload.lists : {};

  // 仅接受合法片单翻译条目，非法条目直接跳过
  const validEntries = {};
  Object.entries(incomingLists).forEach(([id, entry]) => {
    const normalizedId = /^\d+$/.test(String(id).trim())
      ? String(id).trim()
      : "";

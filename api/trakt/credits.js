...
function hasAnyCreditHit(entries) {
  return DOUBAN_TARGET_TYPES.some((type) => {
    const items =
      entries?.[type] && typeof entries[type] === "object" ? entries[type] : {};
...
async function handleGet(req, res, kvConfig) {
  ...
  setResponseCacheHeaders(
    res,
    hasAnyCreditHit(entries) ? CACHE_STATUS.FOUND : CACHE_STATUS.NOT_FOUND,
  );
...
async function handlePost(req, res, kvConfig) {
  ...
  const incomingMovie =
    payload?.movies && typeof payload.movies === "object" ? payload.movies : {};
  const incomingShow =
    payload?.shows && typeof payload.shows === "object" ? payload.shows : {};

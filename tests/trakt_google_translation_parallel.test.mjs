...
test("DeepLX 兼容层切分多行长文本时会尽量合并到接近上限", async () => {
    ...
    try {
        ...
        assert.ok(posts.length > 2);
        assert.ok(requestLengths.every((length) => length <= DEEPLX_MAX_TEXT_CHARACTERS));
        assert.ok(requestLengths.every((length) => length >= DEEPLX_MAX_TEXT_CHARACTERS / 2));
        assert.equal(translatedTexts[0], requestLengths.map((length) => `[${length}]`).join(""));
    } finally {
        globalThis.$ctx = originalContext;
    }
...
test("DeepLX 兼容层会让相同上下文分别跟随每条拼接文本", async () => {
    ...
    try {
        ...
        assert.equal(getRequestText(posts[0].body), "Original Movie (中文电影)\nOverall enjoyable\n¶1¶\nOriginal Movie (中文电影)\nDetailed analysis");

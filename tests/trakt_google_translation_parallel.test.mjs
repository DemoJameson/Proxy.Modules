...
test("DeepLX 兼容层会让相同上下文分别跟随每条拼接文本", async () => {
    const originalContext = globalThis.$ctx;
    const posts = [];
...
test("DeepLX 兼容层会让不同上下文分别跟随对应文本且保持正文顺序", async () => {
    const originalContext = globalThis.$ctx;
    const posts = [];
    ...
    try {
        ...
        assert.equal(posts.length, 1);
        assert.equal(
            getRequestText(posts[0].body),
            "§The Housemaid (家弑服务)§Overall enjoyable\n¶1¶\n§Another Movie (另一部电影)§Slow pacing\n¶2¶\n§The Housemaid (家弑服务)§Detailed analysis",
        );
...
test("DeepLX 兼容层只会按每条文本自己的上下文清理裸上下文残留", async () => {
    const originalContext = globalThis.$ctx;
    const posts = [];
...
test("DeepLX 兼容层支持无上下文、有上下文和重复上下文混合批量拼接", async () => {
    const originalContext = globalThis.$ctx;
    const posts = [];
    ...
    try {
        const translatedTexts = await translateTextsWithGoogle(
            [
                ...
                googleTranslationContext.buildSourceText("Second contextual text", "Movie A (影片甲)"),
            ],
            "en",
        );
        assert.equal(posts.length, 1);
        assert.equal(getRequestText(posts[0].body), "Plain text\n¶1¶\n§Movie A (影片甲)§First contextual text\n¶2¶\n§Movie A (影片甲)§Second contextual text");
        assert.deepEqual(translatedTexts, ["普通文本", "第一条", "第二条"]);
    } finally {
        globalThis.$ctx = originalContext;
    }
});

test("DeepLX 兼容层会清理翻译结果开头残留的孤立上下文边界符", async () => {
    const originalContext = globalThis.$ctx;
...
test("DeepLX 兼容层会修复带冒号标题开头残缺书名号", async () => {
    const originalContext = globalThis.$ctx;
    ...
    globalThis.$ctx = {
        env: {
            http: {
                post() {
                    return Promise.resolve({ status: 200, body: JSON.stringify({ data: "迈克尔-杰克逊：巨星之路》讲述了他的艺术生涯。" }) });
                },
            },
        },
    };

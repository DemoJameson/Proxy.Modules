...
function parseCachedTranslationOverridesStore(value) {
  const parsed = parseRedisJsonValue(value);
  return parsed
    ? normalizeTranslationOverridesStore(parsed)
    : createEmptyTranslationOverridesStore();
}
...
async function readManyEffectiveFromKv(config, mediaType, ids, options = {}) {
  ...
  const [autoResults, translationOverrides] = await Promise.all([
    jsonGetManyKv(config, autoKeys),
    readAllTranslationOverridesFromKv(config),
  ]);
  const entries = {};
...
async function readManyAutoGroupsFromKv(config, groupsByMediaType) {
  ...
  const refs = MEDIA_TYPES.flatMap((mediaType) => {
    ...
    return ids.map((id) => ({
      mediaType,
      id,
      key: buildCacheKey(mediaType, id),
    }));
  });
...
async function readCachePairsFromKv(config, entries) {
  ...
  return entries.map((entry, index) => {
    ...
    return {
      effectiveEntry:
        autoEntry || override ? mergeEntries(autoEntry, override) : null,
      autoEntry,
      override,
    };
  });
...
async function readCachePairFromKv(config, mediaType, id, options = {}) {
  ...
  const [[autoValue], translationOverrides] = await Promise.all([
    jsonGetPairKv(config, [autoKey]),
    readAllTranslationOverridesFromKv(config),
  ]);
  let autoEntry = parseCachedEntry(autoValue);
  if (
    options.hydrateMissingExpiresAt === true &&
    autoEntry &&
    !Object.hasOwn(autoEntry, "expiresAt")
  ) {
    ...
    await pipelineKv(config, [
      ["JSON.SET", autoKey, "$", JSON.stringify(autoEntry)],
    ]);
  }
  const override = translationOverrides[mediaType]?.[id] || null;
...
async function writeManyGroupsToKv(config, groupsByMediaType) {
  ...
  const commands = Object.entries(groupsByMediaType).flatMap(
    ([mediaType, entriesById]) =>
      buildWriteManyCommands(mediaType, entriesById || {}),
  );
  if (commands.length === 0) {
    return;
  }
...
async function deleteCacheEntriesFromKv(config, mediaType, id, target) {
  ...
  if (target === "override" || target === "all") {
    const translationOverrides =
      await readAllTranslationOverridesFromKv(config);
    if (
      translationOverrides[mediaType] &&
      Object.hasOwn(translationOverrides[mediaType], id)
    ) {
      const nextStore = normalizeTranslationOverridesStore({
        ...translationOverrides,
        [mediaType]: Object.fromEntries(
          Object.entries(translationOverrides[mediaType]).filter(
            ([entryId]) => entryId !== id,
          ),
        ),
      });

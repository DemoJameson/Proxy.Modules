...
function normalizeTranslation(translation, emptyAsNull = true) {
  ...
  const normalized = {
    title: isEmptyTranslationValue(translation.title)
      ? null
      : translation.title,
    overview: isEmptyTranslationValue(translation.overview)
      ? null
      : translation.overview,
    tagline: isEmptyTranslationValue(translation.tagline)
      ? null
      : translation.tagline,
  };
...
function normalizeEntry(entry) {
  ...
  const normalized = {
    status:
      entry.status === CACHE_STATUS.FOUND
        ? CACHE_STATUS.FOUND
...
function normalizeImageField(entry, now = Date.now()) {
  ...
  if (status === CACHE_STATUS.PARTIAL_FOUND && url) {
    const expiresAt = Number.isFinite(Number(entry?.expiresAt))
      ...
      : now + PARTIAL_FOUND_TTL_SECONDS * 1000;
    return { status, url, expiresAt };
  }
  const expiresAt = Number.isFinite(Number(entry?.expiresAt))
    ? Number(entry.expiresAt)
    : now + IMAGE_NOT_FOUND_TTL_SECONDS * 1000;
...
function normalizeTranslationOverridesStore(value) {
  ...
  return Object.fromEntries(
    MEDIA_TYPES.map((mediaType) => {
      const entries =
        source[mediaType] && typeof source[mediaType] === "object"
          ? source[mediaType]
          : {};
...
function mergeEntries(autoEntry, override) {
  ...
  const normalizedOverride = override
    ? normalizeTranslationOverrideEntry(override)
    : null;
  const autoTranslation = auto?.translation ? auto.translation : {};
  const translationOverrides = normalizedOverride?.translation
    ? normalizedOverride.translation
    : {};

  const translation = OVERRIDE_FIELDS.reduce((result, field) => {
    const overrideValue = translationOverrides[field];
    result[field] = !isEmptyTranslationValue(overrideValue)
      ? overrideValue
      : autoTranslation[field] || null;
    return result;
  }, {});

  return {
    status: hasUsefulTranslation(translation)
      ? CACHE_STATUS.FOUND
      : CACHE_STATUS.NOT_FOUND,
...
function setResponseCacheHeaders(res, status) {
  const headers =
    RESPONSE_CACHE_HEADERS[status] ||
    RESPONSE_CACHE_HEADERS[CACHE_STATUS.NOT_FOUND];

...
function buildWriteManyImageCommands(group, entriesById, mode = "chinese") {
  ...
  Object.entries(entriesById).forEach(([id, rawEntry]) => {
    ...
    const hasNotFound = fields.some(
      (field) => field?.status === CACHE_STATUS.NOT_FOUND,
    );
    const hasPartialFound = fields.some(
      (field) => field?.status === CACHE_STATUS.PARTIAL_FOUND,
    );
    if (hasNotFound) {
...
async function writeManyImageGroupsToKv(config, groupsByGroup) {
  ...
  const modes =
    groupsByGroup?.modes && typeof groupsByGroup.modes === "object"
      ? groupsByGroup.modes
      : null;
  const commands = (modes ? Object.entries(modes) : []).flatMap(
    ([mode, groups]) =>
      Object.entries(groups || {}).flatMap(([group, entriesById]) =>
        IMAGE_GROUPS.includes(group)
          ? buildWriteManyImageCommands(group, entriesById || {}, mode)
          : [],
      ),
  );

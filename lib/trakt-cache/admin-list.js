...
function getTraktSearchEntry(result, allowedMediaTypes) {
  ...
  const mediaType =
    resultType === "movie" ? "movies" : resultType === "show" ? "shows" : "";
  ...
  const normalizedId =
    Number.isFinite(id) || typeof id === "string" ? String(id) : "";
...
async function searchTraktTitleIds(queryText, types) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 401 || response.status === 403) {
      throw new Error(
        `Trakt search HTTP ${response.status}. Check TRAKT_API_KEY is the Trakt app client id.${suffix}`,
      );
    }
  ...
  const candidates = Array.isArray(body)
    ? body
        .map((result) => getTraktSearchEntry(result, target.mediaTypes))
        .filter(Boolean)
    : [];
...
function matchesOverrideFilter(item, override) {
  ...
  const hasOverride = !!(
    item.override?.translation &&
    Object.keys(item.override.translation).length > 0
  );
...
function matchesStatusFilter(item, status) {
  ...
  const entryStatus = item.effectiveEntry
    ? item.effectiveEntry.status
...
function parseAdminListCursor(value, types) {
  ...
  if (raw === "0") {
    return {
      ...Object.fromEntries(
        types.map((type) => [type, { autoCursor: "0", overrideCursor: "0" }]),
      ),
      __pending: [],
    };
  }
...
function formatAdminListCursor(cursorByType, types, pendingEntries = []) {
  ...
  const pending = pendingEntries.filter(
    (entry) => entry && types.includes(entry.type) && entry.id,
  );
  if (
    pending.length === 0 &&
    Object.values(normalized).every(
      (cursor) => cursor.autoCursor === "0" && cursor.overrideCursor === "0",
    )
  ) {
    return "0";
  }
  return Buffer.from(
    JSON.stringify({ ...normalized, __pending: pending }),
    "utf8",
  ).toString("base64url");
}
...
function compareAdminSearchEntries(left, right) {
  const typeDelta =
    MEDIA_TYPES.indexOf(left.type) - MEDIA_TYPES.indexOf(right.type);
  if (typeDelta !== 0) {
    return typeDelta;
  }
...
async function listCacheItemsFromKv(config, options) {
  ...
  const idEntries = [];
  const seen = new Set();
  const pendingEntries = (cursorByType.__pending || []).filter(
    (entry) => entry && types.includes(entry.type) && entry.id,
  );
  ...
  const candidates = idEntries
    ...
    .filter(({ item }) =>
      matchesOverrideFilter(item, options.override || "all"),
    )
    .filter(({ item }) => matchesStatusFilter(item, options.status || "all"));
  ...
  return {
    items,
    cursor:
      nextPendingEntries.length > 0
        ? formatAdminListCursor(nextCursorByType, types, nextPendingEntries)
        : "0",
    limit,
  };
}
